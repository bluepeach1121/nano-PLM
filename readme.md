...
The LLM is meant to:
- Ensure I had the prequisite theory knowlegde before I started coding, by asking questions, and providing appropriate literature.
...
- In `tokeniser.py`, A fixed protein residue tokenizer rather than BPE (given that the vocab size isnt much). The vocabulary contains `[PAD]`, `[MASK]`, `[CLS]`, the 20 canonical amino acids, and `X`. Ambiguous/rare residue symbols `B/Z/J/U/O/X` are mapped to `X`. No invalid characters are found.
...
python model.py
```
Note: this downloads the file if its not available.
